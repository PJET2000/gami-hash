...
run_gui() { # scenario root output [extra env]
  local scen="$1" log="$2"
  : > "$log"; : > "$WORK/state"
...
grep -q "Restzeit" "$WORK/g1.log" && ok "ETA shown in progress text" || fail "no ETA in progress text"
grep -q "Fertig" "$WORK/g1.log" && ok "done message shown (German)" || fail "no done dialog"
grep -q "Ordner öffnen" "$WORK/g1.log" && ok "done dialog offers 'Ordner öffnen'" || fail "no open-folder button"
grep -q "Prüfsummen-Liste" "$WORK/g1.log" && ok "welcome text is German" || fail "welcome not German"
grep -q "kaputt machen" "$WORK/g1.log" && ok "reassurance wording present" || fail "reassurance missing"
grep -q "Ruhezustand" "$WORK/g1.log" && ok "sleep-mode reassurance in start dialog" || fail "sleep reassurance missing"
...
echo "== G3: cancel during progress → resume via GUI =="
OUT3="$WORK/out/gui3.csv"
run_gui cancel-progress "$WORK/g3.log" "$OUT3"; RC=$?
[ $RC -eq 0 ] && ok "clean exit after cancel" || fail "exit $RC"
[ -f "$OUT3.part.json" ] && ok "checkpoint kept" || fail "no checkpoint"
grep -q "angehalten" "$WORK/g3.log" && ok "German 'stopped' message" || fail "no stopped message"
# now resume: scenario 'resume' answers OK (=Fortsetzen) on the resume question
run_gui resume "$WORK/g3b.log" "$OUT3"; RC=$?
[ $RC -eq 0 ] && ok "resume run exits 0" || fail "resume exit $RC"
grep -q "Fortsetzen" "$WORK/g3b.log" && ok "resume question shown with Fortsetzen" || fail "no resume question"
grep -q -- "--extra-button" "$WORK/g3b.log" && ok "start-over offered as extra button" || fail "no extra button"
...
grep -q "bereits aus dem vorherigen Durchlauf" "$WORK/g3b.log" && ok "done message mentions resumed files" || fail "no resumed-files note"
...
run_gui cancel-progress "$WORK/g5.log" "$OUT5"
...
EOF
run_gui resume-other-root "$WORK/g5b.log" "$OUT5"; RC=$?
[ $RC -eq 0 ] && ok "resume-anyway run exits 0" || fail "exit $RC"
grep -q "damals unter dem Pfad" "$WORK/g5b.log" && ok "drive-letter explanation shown" || fail "no explanation dialog"
grep -q "Laufwerksbuchstaben" "$WORK/g5b.log" && ok "wording mentions drive letters" || fail "wording missing"
